...
def refresh_vban():
    try:
        ...
        return jsonify({'error': str(e)}), 400
@app.route('/snap_detected') # Modified from clap_detected
def snap_detected():
    socketio.emit('snap', {'message': 'Claquement de doigts détecté!'})
...
def validate_settings(settings):
    ...
    return True
@socketio.on('snap_detected')
def handle_snap(data):
    print(f"🎯 Snap detected: {data}") # Debug log
    try:
        socketio.emit('snap', {
            'source_id': 'microphone',
            'timestamp': time.time()
        }, broadcast=True)
        print(f"✅ Snap event emitted")
    except Exception as e:
        print(f"❌ Error emitting snap event: {str(e)}")
