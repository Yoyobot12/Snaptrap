...
class AudioDetector:
    ...
    def _handle_result(self, result, timestamp):
        ...
        try:
            ...
            # Calculer le score pour la détection de finger snap
            score_sum = sum(
                category.score
                for category in classification.categories
            ...
            if score_sum > 0.1:  # Abaisser le seuil pour le debug
                logging.debug(f"Score de snap calculé pour source {source_id}: {score_sum}")
            ...
            # Vérifier si on a détecté un snap
            current_time = time.time()
            if score_sum > 0.25 and (current_time - self.last_detection_time.get(source_id, 0)) > 1.0:  # Seuil ajusté pour snaps
