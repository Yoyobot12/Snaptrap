    ...
    try:
        ...
        def create_detection_callback(source_name, webhook_url=None):
            def handle_detection(detection_data):
                try:
                    logging.info(f"SNAP détecté sur {source_name} avec score {detection_data['score']}")
                    if socketio:
                        socketio.emit('snap', {
                    ...
                    logging.error(f"Erreur lors de l'envoi de l'événement snap pour {source_name}: {str(e)}")
